...
fetch(window.location.origin + "/api/users/" + k_userId)
    .then(JSONdata => JSONdata.json().then(res => {
    if (res.success) {
...
function updateUser() {
    createAlertMultipleBtns("Update Information", "sure?", "OK", () => saveUser(), "Cancel", null);
}
function deleteUser() {
    createAlertMultipleBtns("Delete User", "sure?", "OK", () => {
        userDelete(k_userId);
    }, "Cancel", null);
}
function saveUser() {
