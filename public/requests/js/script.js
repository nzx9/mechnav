...
let timeout_1 = null;
userLocation.getRealTimeLocation(true, (err, pos) => {
    if (!err) {
        ...
        if (err)
            ...
            get(window.location.origin +
                ...
                if (res.data.length === 0)
                    createAlert("Sorry", "No mecahnics nearby.", "Retry", () => showMecanicsNearby(), "OK", () => {
                        window.location.href = origin;
                    });
                else {
                    res.data.forEach((element, idx, array) => __awaiter(this, void 0, void 0, function* () {
                        yield get(window.location.origin + `/api/users/${element.mechanicId}`)
                            .then(_res => {
                            if (_res.success) {
                                dispData += `<ion-item>
                      <ion-avatar slot="start">
                        <ion-img src="${_res.data.profilePic === null
                                    ? "https://cdn.onlinewebfonts.com/svg/img_568656.png"
                                    : _res.data.profilePic}"/>
                      </ion-avatar>
                      <ion-label>
                        <h2>${_res.data.username}</h2>
                        <p>${element.distance < 1
                                    ? (element.distance * 1000).toFixed(0) + "m"
                                    : element.distance.toFixed(2) + "km"} away from you</p>
                        <p>Tel: ${_res.data.contactNumber} </p>
                      </ion-label>
                      <ion-button id="${element.mechanicId} ${element.distance}" onclick="requestMechanic(this.id)" color="warning">Request</ion-button>
                    </ion-item>`;
                            }
      ...
      ${window.location.origin}/api/jobs/${userId}&client&true`)
        ...
        if (result.success === true && result.data.size === 0) {
            ...
            timeout_1 = setTimeout(() => {
                dismissLoading();
                createAlert("Response error", "Sorry, No response from Mechanic.", "Ok");
            }, 10000);
        }
        else {
            dismissLoading();
            createAlert("Error", "You have ongoing Job.Please cancel or finish it before request new one", "OK");
        }
    })
        .catch(err => {
        dismissLoading();
...
socket.on("responsefrommechanic_server", (res) => {
