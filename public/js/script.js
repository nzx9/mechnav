...
socket.on("requestfromclient_server", msg => {
    if (msg.mechanicId === userId) {
        get(`
      ${window.location.origin}/api/jobs/${userId}&mechanic&true`, (err, data) => {
            if (err) {
                ...
                if (data.data.size === 0) {
                    createAlertMultipleBtns("Client Request", `Client is ${msg.distance < 1
                        ? (msg.distance * 1000).toFixed(0) + "m"
                        : msg.distance.toFixed(2) + "km"} away from you`, "Accept", () => {
                        socket.emit("responsefrommechanic", {
                            success: true,
...
socket.on("canceljob_server", data => {
    if (data.clientId === userId && accountType === "client") {
        createAlert("Job Canceled", "Sorry, Your Job has canceled by Mechanic", "OK", () => window.location.reload(), false);
    }
    if (data.mechanicId === userId && accountType === "mechanic") {
        createAlert("Job Canceled", "Client canceled Job", "OK", () => window.location.reload(), false);
    }
});
// get information from server
fetchAndShowInfo();
//socket communication in between client and server && track user
userLocation.getRealTimeLocation(true, (err, pos) => __awaiter(void 0, void 0, void 0, function* () {
    if (!err) {
...
userLocation.getLocationWithInterval(5, (err, pos) => __awaiter(void 0, void 0, void 0, function* () {
    if (!err) {
        if (jobData.endPoint !== null) {
            directionsService.route({
                origin: new google.maps.LatLng(pos.coords.latitude, pos.coords.longitude),
                destination: new google.maps.LatLng(jobData.endPoint.lat, jobData.endPoint.lng),
...
function fetchAndShowInfo() {
    get(window.location.href + "api/users/" + userId, (err, res) => {
        if (err) {
            createAlert("Network Error", "Can't fetch data.", "Retry", () => fetchAndShowInfo());
        }
        else {
            if (res.success !== 1) {
                accountType = res.data.accountType;
                //load routes if available
                getJobDataFromServer(userId, accountType, (data) => {
                    if (data.success && data.data.size !== 0) {
                        ...
                        setRoute(data.data.body.startPoint, data.data.body.endPoint, mapdata => {
                            if (accountType === "mechanic") {
                                get(window.location.origin + "/api/users/" + jobData.clientId, (err, clidata) => {
                                    if (!err) {
                                        if (clidata.success) {
                                ...
                                createToastPlain_2(`${mapdata.steps[0].instructions}<b>ETA : ${mapdata.duration.text}(${mapdata.distance.text})</b>`, null, "bottom", false, "toast-up");
                            }
                            else if (accountType === "client") {
                                get(window.location.origin + "/api/users/" + jobData.mechanicId, (err, mechdata) => {
                                    if (!err) {
                                        if (mechdata.success) {
                                ...
                                createToastPlain_2(`${mapdata.distance.text} away from you. ETA ${mapdata.duration.text}`, null, "bottom", false, "toast-up");
                            }
                        });
                    }
                });
...
function getJobDataFromServer(Id, accountType, callback) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      ${window.location.origin}/api/jobs/${Id}&${accountType}&true`)
            ...
            .catch(err => createAlert("Network error", "Can't fetch currently active job", "Retry", () => {
            console.log(err);
...
function setRoute(start, end, callback) {
    let request = {
        origin: start,
        destination: end,
        travelMode: google.maps.TravelMode.DRIVING
...
function setCMMarker(lat, lng, accType) {
    if (accType === "mechanic") {
...
const btn_locate_me_follow_icon = document.querySelector("#btn-locate-me-follow-icon");
...
function showPopOver(event, layout) {
    return __awaiter(this, void 0, void 0, function* () {
        let popover = yield popoverController.create({
            component: layout,
            event: event,
            translucent: true
        });
...
// pop over
customElements.define("options-layout-client", class ModalContent extends HTMLElement {
    connectedCallback() {
        this.innerHTML = `
...
customElements.define("options-layout-admin", class ModalContent extends HTMLElement {
    connectedCallback() {
        this.innerHTML = `
            <ion-list style="z-index : 1 !important">
            <ion-list-header>Options</ion-list-header>
...
function JobCancelingToast(details, accType) {
    return __awaiter(this, void 0, void 0, function* () {
        const toast = document.createElement("ion-toast");
        toast.message = `${accType === "client" ? "Client: " : "mechanic" ? "Mechanic: " : null} ${details.data.username}\nTel: ${details.data.contactNumber}`;
        toast.position = "top";
        ...
        toast.buttons = [
            {
                text: "Cancel Job",
                handler: () => {
                    createAlertMultipleBtns("Conformation?", "Do you want to cancel this job?", "No", () => fetchAndShowInfo(), "Yes", () => {
                        if (jobData.jobId !== null) {
                            finishJob(jobData.jobId, (err, res) => {
                                if (err) {
                                    createAlert("Error", err.toString(), "Retry", () => finishJob(jobData.jobId, (err, res_) => {
                                        if (err) {
...
function showDirectionDataBar(dirdata) {
    ...
    if (mapdata.distance.value >= 10) {
        ...
        if (accountType === "mechanic" && !showedOnce) {
            showedOnce = true;
            createAlert("Confirmation", "Did you arrive at the destination?", "Yes", () => {
                finishJob(jobData.jobId, (err, res) => {
                    if (err) {
                        createAlert("Error", err.toString, "OK", null, false);
                        fetchAndShowInfo();
